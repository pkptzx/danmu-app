...
fn main() -> Result<()>{
    ...
    //     }
    // });
    clipboard.write_image(image.width(), image.height(), image.as_raw())?;
    // let rgba = img.to_rgba8();
    // let pixel0 = &rgba.get_pixel(0, 0);
    // println!("pixel: {:?}", pixel0);
    // clipboard.write_image(img.width(), img.height(), rgba.as_raw()).unwrap();
